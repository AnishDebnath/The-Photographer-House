...
export const ChatWidget: React.FC = () => {
    ...
    const handleSendMessage = (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            const reply: Message = {
                id: Date.now() + 1,
                text: "Thank you for reaching out! One of our team members will get back to you shortly. For immediate assistance, feel free to call us at +91 8910092451.",
                sender: 'team',
                timestamp: new Date()
            };
    ...
    return (
        <div className="fixed bottom-6 inset-x-0 z-[50] pointer-events-none">
            <div className="w-full flex flex-col items-center sm:items-end relative px-4 md:px-8">
                {/* Widget Container */}
                <div className={`absolute bottom-20 w-[calc(100vw-2rem)] sm:w-[350px] max-h-[75vh] sm:max-h-none bg-white dark:bg-zinc-950 rounded-3xl sm:rounded-[2rem] shadow-2xl transition-all duration-500 origin-bottom sm:origin-bottom-right overflow-hidden border border-gray-100 dark:border-white/10 ${isOpen ? 'scale-100 opacity-100 translate-y-0 pointer-events-auto' : 'scale-90 opacity-0 translate-y-10 pointer-events-none'}`}>
                    {view === 'entry' ? (
                        <>
                            {/* Entry View Header */}
                        ...
                        /* Chatting View */
                        <div className="flex flex-col h-[450px] sm:h-[500px] max-h-[70vh] sm:max-h-none bg-white dark:bg-zinc-950">
                            {/* Chat Header */}
                ...
                <div className="w-full flex justify-end">
                    <button
                        onClick={toggleOpen}
                        className={`w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-500 hover:scale-110 active:scale-95 z-[60] border border-white/10 relative pointer-events-auto ${isOpen ? 'bg-black text-gold-500 rotate-180 shadow-black/50' : 'bg-gold-500 text-black shadow-gold-500/40'}`}
